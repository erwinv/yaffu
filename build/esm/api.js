...
export function genericCombine(inputs, outputPath) {
    const graph = new FilterGraph(inputs);
    compositeGrid(graph, ['vout']);
    mixAudio(graph, ['aout']);
    return graph.map(['vout', 'aout'], outputPath);
}
export function mixAudio(graph, outputIds, delays = []) {
...
export function renderCamWithThumbnail(graph, outputId) {
...
export function compositeGrid(graph, outputIds) {
    ...
    if (numTilesOnBottomRow > 0) {
        const botRowTilesIds = _.range(numTilesOnBottomRow).map((i) => `tile${numTilesOnTopGrid + i}`);
        graph
...
export function compositePresentation(graph, mainId, othersIds, outputId) {
