...
export async function probe(path, entry) {
...
export async function concatDemux(clipPaths, outputPath) {
    const concatListPath = join(dirname(outputPath), basename(outputPath, extname(outputPath)) + '_concatList.txt' // TODO FIXME use os.tmpdir()
    );
    await writeFile(concatListPath, clipPaths.map((clipPath) => `file ${clipPath}`).join('\n'));
...
export async function mux(graph, verbose = true) {
